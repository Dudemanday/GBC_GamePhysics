...
bool CollisionManager::circleBoxCheck(SDL_Rect circle, SDL_Rect rect)
{
	circle.w *= 0.8;
	rect.w *= 0.8;
	rect.h *= 0.8;
	...
	if (circleDistance.x > (rect.w / 2 + circle.w)) { return false; }
	if (circleDistance.y > (rect.h / 2 + circle.w)) { return false; }

	if (circleDistance.x <= (rect.w / 2)) { return true; }
	if (circleDistance.y <= (rect.h / 2)) { return true; }

	float cornerDistance_sq = pow(circleDistance.x - rect.w / 2, 2.0f) +
		pow(circleDistance.y - rect.h / 2, 2.0f);

	return (cornerDistance_sq <= (circle.w ^ 2));
}
...
bool CollisionManager::circleCircleCheck(glm::vec2 pos1, glm::vec2 pos2, float r1, float r2)
{
	float x1 = pos1.x;
	float x2 = pos2.x;
	float y1 = pos1.y;
